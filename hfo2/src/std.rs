...
unsafe fn check_or_fill(cond: bool, dest: *const c_void, destsz: size_t, ch: i32, condmsg: &str) {
    if !cond {
        if !dest.is_null() && destsz <= RSIZE_MAX {
            unsafe { memset_s(dest, destsz, ch, destsz) };
        }
...
unsafe fn check_or_fill_zero(cond: bool, dest: *const c_void, destsz: size_t, condmsg: &str) {
    unsafe { check_or_fill(cond, dest, destsz, 0, condmsg) }
}

#[no_mangle]
pub unsafe extern "C" fn memset_s(dest: *const c_void, destsz: size_t, ch: c_int, count: size_t) {
    unsafe { check_or_fill(!dest.is_null(), dest, destsz, ch, "!dest.is_null()") };

    // Check count <= destsz <= RSIZE_MAX.
    unsafe { check_or_fill(destsz <= RSIZE_MAX, dest, destsz, ch, "destsz <= RSIZE_MAX") };
    unsafe { check_or_fill(count <= destsz, dest, destsz, ch, "count <= destsz") };

    unsafe { ptr::write_bytes(dest as *mut u8, ch as u8, count) };
}
...
pub unsafe extern "C" fn memcpy_s(
    ...
    unsafe { check_or_fill_zero(!dest.is_null(), dest, destsz, "!dest.is_null()") };
    unsafe { check_or_fill_zero(!src.is_null(), dest, destsz, "!src.is_null()") };

    // Check count <= destsz <= RSIZE_MAX.
    unsafe { check_or_fill_zero(destsz <= RSIZE_MAX, dest, destsz, "destsz <= RSIZE_MAX") };
    unsafe { check_or_fill_zero(count <= destsz, dest, destsz, "count <= destsz") };

    // Buffer overlap test.
    // case a) `d < s` impiles `s >= d+count`
    // case b) `d > s` impiles `d >= s+count`
    unsafe { check_or_fill_zero(d != s, dest, destsz, "d != s") };
    unsafe {
        check_or_fill_zero(
            d < s || d >= (s + count),
            dest,
    ...
    unsafe { ptr::copy_nonoverlapping(src as *const u8, dest as *mut u8, count) };
}
...
pub unsafe extern "C" fn memmove_s(
    ...
    unsafe { ptr::copy(src as *const u8, dest as *mut u8, count) };
}
