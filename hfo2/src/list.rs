...
pub unsafe fn list_append(l: *mut ListEntry, e: *mut ListEntry) {
    unsafe { (*e).next = l };
    unsafe { (*e).prev = (*l).prev };

    unsafe { (*(*e).next).prev = e };
    unsafe { (*(*e).prev).next = e };
}

#[inline]
pub unsafe fn list_prepend(l: *mut ListEntry, e: *mut ListEntry) {
    unsafe { (*e).next = (*l).next };
    unsafe { (*e).prev = l };
...
pub unsafe fn list_empty(l: *const ListEntry) -> bool {
    unsafe { (*l).next as *const _ == l }
}

#[inline]
pub unsafe fn list_remove(e: *mut ListEntry) {
    unsafe { (*(*e).prev).next = (*e).next };
    unsafe { (*(*e).next).prev = (*e).prev };
    unsafe {
        list_init(e);
    }
}
